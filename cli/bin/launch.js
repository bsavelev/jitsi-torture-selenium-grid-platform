...
const commands = Array(Number(NODE_SIZE)).fill(null).map((u, i) => `docker run --name grid_${i} -d -p ${Number(NODE_PORT) + i}:5555 -e HUB_HOST=${HUB_HOST} -e HUB_PORT=${HUB_PORT} -e REMOTE_HOST=http://${LOCAL_IP}:${Number(NODE_PORT) + i} -v /dev/shm:/dev/shm -v /usr/share/jitsi:/usr/share/jitsi ${image}`);
